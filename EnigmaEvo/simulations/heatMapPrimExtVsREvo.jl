...
function simulation_primExtVsREvo(onlyPlots=false,fromResults=false)   #supposedly its better to wrap stuff in functions and not use global variables if possible(not sure if typed ones are a problem)
    ...
    simulationName = "heatmapPrimExtVsREvo";        #specify the name of the simulation
    ...
    if !fromResults
        ...
        @sync @distributed for (primInd,rPrimExt,rEvoInd,rEvo,repetition) in loop_vars
            if onlyPlots
                ...
                rates0 = (rc = 1., rprimext = rPrimExt, rsecext = 10., reo = 1., revo = rEvo, rext = 0.16);
    ...
    nFinishedRunsPlot = Plots.heatmap(string.(rEvoVals), string.(primVals)dropdims(sum(runFinished,dims=3),dims=3),
    size = (720,720), xlabel = "evolutionary rate", ylabel = "primary extinction rate",
    ...
    for vecZParam in endVecZParams
        maxPlot = Plots.heatmap(string.(rEvoVals), string.(primVals)dropdims(mean(heatMapsMax[vecZParam],dims=3),dims=3),
        size = (720,720), xlabel = "evolutionary rate", ylabel = "primary extinction rate",
        ...
        if any(isnan,heatMapsMax[vecZParam])
            maxPlot = Plots.heatmap(string.(rEvoVals), string.(primVals)NaNStatistics.nanmean(heatMapsMax[vecZParam],dim=3),
            size = (720,720), xlabel = "evolutionary rate", ylabel = "primary extinction rate",
    ...
    for zParam in zParams
        meanPlot = Plots.heatmap(string.(rEvoVals), string.(primVals)dropdims(mean(heatMapsMean[zParam],dims=3),dims=3),
            size = (720,720), xlabel = "evolutionary rate", ylabel = "primary extinction rate",
        ...
        if any(isnan,heatMapsMean[zParam])
            meanPlot = Plots.heatmap(string.(rEvoVals), string.(primVals)NaNStatistics.nanmean(heatMapsMean[zParam],dim=3),
            size = (720,720), xlabel = "evolutionary rate", ylabel = "primary extinction rate",
