...
""""
    rEvoLinePlotConstGlob(onlyPlots=false,fromResults=false) 

    Performs a series of simulations varying over the evolutionary rate with a constant global extinction rate. Creates line plots of results.
...
"""
function rEvoLinePlotConstGlob(onlyPlots=false,fromResults=false)   #supposedly its better to wrap stuff in functions and not use global variables if possible(not sure if typed ones are a problem)
    ...
    simulationName = "rEvoLinePlotconstGlobExt";        #specify the name of the simulation
    ...
    logging,S,lambda,SSprobs,SOprobs,nBasalRes,primVals,nRepets, secVals,evoVals)
    ...
    if !fromResults
        ...
        @sync @distributed for (primInd,rPrimExt,secInd,rSecExt,evoInd,rEvo,repetition) in loop_vars
            ...
            if onlyPlots
                ...
                rates0 = (rc = 1., rprimext = rPrimExt, rsecext = rSecExt, reo = 1., revo = rEvo, rext = .0397);
