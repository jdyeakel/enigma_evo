function intmatrixv5(S::Int64, lambda::Float64, SSprobs, SOprobs, OOprobs)::Tuple{Array{Int64}, Array{Int64}, Array{Int64}}
    ...
    # NOTE: Edgelist version
    edgelist = Array{Int64}(undef,0,3);
    # NOTE: 3rd column will save ID - here we will save 1 = eat, 2 = need, 3 = make
    ...
    estengineeringlinks = function(edgelist)
    for i = 1:E
        for j=1:O
            if obindpS[i,j] == 1
                edgelist = intadd(edgelist,[engind[i] obind[j]],[3 2]);
    ...
    edgelist = estengineeringlinks(edgelist);

    #Calculate the distribution of trophic links per species

    #NOTE: won't need these now
    ...
    # intm[2,1] = 'e'
    # tp_m[2,1] = 1;
    edgelist = intadd(edgelist,[2 1],1)
    ...
    #NOTE: now will keep track of IDs through [spcid, ocid, espcid, cid]
    # diagindices = diagind(intm);

    # intm[diagindices[2:S]] .= Ref('n');
	# 	intm[diagindices[S+1:N]] .= Ref('i');

    #NOTE:Don't need to specify ignore interaction anymore
    ...
    function estinteractionlinks(edgelist::Array{Int64},prob_line::Array{Float64},S::Int64)::Array{Int64}
        for i = 2:S
            for j = 2:S
                ...
                if i > j
                    ...
                    if r_draw < prob_line[1]
                        ...
                        if rr_draw < 0.5
                            ...
                            edgelist = intadd(edgelist,[i j],[1 2])
                            ...
                            #   intm[j,i] = 'e';
                            #   tp_m[j,i] = 1;
                            edgelist = intadd(edgelist,[i j],[2 1])
                        end
                    end
                    ...
                    if r_draw > prob_line[1] && r_draw < prob_line[2]
                        ...
                        #   mp_m[i,j] = 1;
                        #   mp_m[j,i] = 1;
                        edgelist = intadd(edgelist,[i j],[2 2])
                    end
                    ...
                    if r_draw > prob_line[2] && r_draw < prob_line[3]
                        ...
                        if rr_draw < 0.5
                            ...
                            # intm[j,i] = 'n';
                            # mp_m[j,i] = 1;
                            #NOTE: only adds interaction (not ignore)
                            edgelist = intadd(edgelist,[j i],2)
                        else
                            # intm[i,j] = 'n';
                            # mp_m[i,j] = 1;
                            # intm[j,i] = 'i';
                            edgelist = intadd(edgelist,[i j],2)
                        end
                    end
                    ...
                    if r_draw > prob_line[3] && r_draw < prob_line[4]
                        ...
                        if rr_draw < 0.5
                            ...
                            # tp_m[i,j] = 1;
                            # intm[j,i] = 'i';
                            edgelist = intadd(edgelist,[i j],1)
                        else
                            # intm[i,j] = 'i';
                            # intm[j,i] = 'e';
                            # tp_m[j,i] = 1;
                    ...
                    if r_draw > prob_line[5] && r_draw < prob_line[6]
                        ...
                        #   tp_m[i,j] = 1;
                        #   tp_m[j,i] = 1;
                        edgelist = intadd(edgelist,[i j],[1 1])
                    end
    ...
    edgelist = estinteractionlinks(edgelist,prob_line,S);


    #We could assume that any species without recorded trophic interactions is a primary producer
    trophicints = findall(x->x==1,edgelist[:,3]);
    # Which species do not have trophic interactions?
    prim_prod = setdiff(collect(2:S),unique(edgelist[trophicints,1]));
    function estinteractionprimproducers(edgelist,prim_prod)
        for i=1:length(prim_prod)
            edgelist = intadd(edgelist,[prim_prod[i] 1],1)
        end
        return edgelist
    end
    edgelist = estinteractionprimproducers(edgelist,prim_prod);
    ...
    function estobjinteractionlinks(edgelist::Array{Int64},so_prob_line::Array{Float64},S::Int64,N::Int64)::Array{Int64}
        for i=2:S
            for j=S+1:N
                # if intm[i,j] == '0'
                    r_draw = rand();
    ...
    edgelist = estobjinteractionlinks(edgelist,so_pw_prob,S,N);
    ...
    function estobjobjinteractionlinks(edgelist::Array{Int64},oo_prob_line::Array{Float64},S::Int64,N::Int64)::Array{Int64}
        for i=(S+1):N
            for j=(S+1):N
                # if intm[i,j] == '0'
                    r_draw = rand();
